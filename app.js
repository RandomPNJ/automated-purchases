...
// ABI of the router
const routerAbi = new ethers.utils.Interface(require("./abi.json"));
const EXPECTED_PONG_BACK = 30000;
const KEEP_ALIVE_CHECK_INTERVAL = 15000;
const provider = new ethers.providers.WebSocketProvider(
  process.env.NODE_WSS
);
// const provider = new ethers.providers.JsonRpcProvider(process.env.PUBLIC_PROVIDER);
const wallet = new ethers.Wallet(process.env.PRIVATE_KEY);
const account = wallet.connect(provider);
const router = new ethers.Contract(tokens.router, routerAbi, account);
        ...
        if (tx && tx.to) {
          if (tx.to === tokens.router) {
            ...
            if ((liqProvidedInETH || re2.test(tx.data)) && buyTxSend < tokens.nbSwap) {
              const decodedInput = routerAbi.parseTransaction({
                data: tx.data,
                value: tx.value,
              });
